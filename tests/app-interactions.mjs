...
let root, window, act;
try {
    await build({ root: project, configFile: `${project}/vite.config.ts`, logLevel: 'error', build: { ssr: `${project}/src/App.tsx`, outDir: output, emptyOutDir: true, minify: false } });
    process.env.NODE_ENV = 'development';
    window = new Window({ url: 'http://localhost/' });
    window.matchMedia = query => ({ matches: query.includes('hover: hover'), media: query, addListener() {}, removeListener() {}, addEventListener() {}, removeEventListener() {} });
    window.document.documentElement.lang = 'en';
    window.localStorage.setItem('drakko-display', JSON.stringify({ calm: true }));
    for (const name of ['window', 'document', 'navigator', 'HTMLElement', 'HTMLInputElement', 'HTMLButtonElement', 'Element', 'SVGElement', 'Node', 'NodeFilter', 'DocumentFragment', 'MutationObserver', 'ResizeObserver', 'CustomEvent', 'Event', 'FocusEvent', 'KeyboardEvent', 'MouseEvent', 'PointerEvent', 'getComputedStyle', 'requestAnimationFrame', 'cancelAnimationFrame', 'localStorage', 'CSS']) {
    ...
    const interact = async action => { await act(action); await act(settle); };
    const press = async (element, key) => interact(() => { element.dispatchEvent(new window.KeyboardEvent('keydown', { key, bubbles: true, cancelable: true })); });
    const click = async element => interact(() => { assert.ok(element, 'click target exists'); element.click(); });
    const audit = async name => {
        // Happy DOM has no layout. These are semantic checks, not a browser WCAG audit.
        ...
        assert.deepEqual(result.violations.map(v => ({ id: v.id, nodes: v.nodes.map(n => n.html) })), [], `${name}: semantic axe violations`);
        console.log(`PASS: ${name}, axe ${result.passes.length} passed rules; manual review: ${result.incomplete.map(v => v.id).join(', ') || 'none returned'}.`);
        for (const issue of result.incomplete) console.log(JSON.stringify({ rule: issue.id, nodes: issue.nodes.map(node => ({ html: node.html, reason: node.failureSummary })) }));
    };
    ...
    assert.match(document.querySelector('[role=log]').textContent, /github.com\/Drakko99\/juego_impostor/);
    await press(input, 'ArrowUp'); assert.equal(input.value, 'projects');
    await press(input, 'ArrowUp'); assert.equal(input.value, 'about');
    await press(input, 'ArrowDown'); assert.equal(input.value, 'projects');
    await press(input, 'ArrowDown'); assert.equal(input.value, '');
    await act(async () => {
        Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype, 'value').set.call(input, '<script>alert(1)</script>');
        input.dispatchEvent(new window.Event('input', { bubbles: true }));
    });
    await act(async () => document.querySelector('.terminal-form').dispatchEvent(new window.Event('submit', { bubbles: true, cancelable: true })));
    ...
    assert.equal(document.getElementById(contact.getAttribute('aria-controls'))?.getAttribute('role'), 'menu');
    ...
    assert.equal(document.getElementById(display.getAttribute('aria-controls'))?.getAttribute('role'), 'menu');
    const contrast = [...document.querySelectorAll('[role=menuitemcheckbox]')].find(n => n.textContent.includes('High contrast'));
    ...
    await press(document.activeElement, 'Escape');
    console.log('PASS: menu keyboard navigation, Escape focus restoration and persisted display settings.');

    for (const [route, heading] of [['/projects', 'Projects of my own.'], ['/stack', 'My stack.'], ['/experience', 'Experience']]) {
        ...
        if (route === '/projects') {
            assert.ok(document.querySelector('a[href="https://github.com/Drakko99/game-library-mobile"]'));
            ...
            assert.equal(document.querySelectorAll('.project-links a[href^="https://github.com/"]').length, 4);
            assert.equal(document.querySelectorAll('.project-links a[href^="https://play.google.com/"]').length, 1);
        }
        await audit(route);
    }
    ...
    await act(async () => { window.history.pushState({}, '', '/projects#%invalid'); window.dispatchEvent(new window.PopStateEvent('popstate')); await settle(); });
    assert.ok(document.querySelector('.projects-page'));
    await act(async () => { window.history.pushState({}, '', '/missing'); window.dispatchEvent(new window.PopStateEvent('popstate')); await settle(); });
    assert.ok(document.querySelector('.not-found'));
    await audit('404');
    await click(document.querySelector('.not-found a'));
    assert.equal(window.location.pathname, '/');
    console.log('PASS: all routes, titles, focus, repo/store links, mobile menu, hash links, invalid fragment and 404 recovery. No browser layout assertions.');
} finally {
