...
function TimelineCard({ exp }: { exp: ExperienceItem }) {
    ...
    return (
        ...
        >
            <h2 className="timeline-card-title">{exp.title}</h2>
            <p className="timeline-company"><FiBriefcase className="shrink-0 text-secondary" aria-hidden="true" />{exp.company}</p>
            ...
            <motion.div id={detailsId} inert={!expanded} aria-hidden={!expanded} initial={false} animate={{ height: expanded ? 'auto' : 0, opacity: expanded ? 1 : 0 }} transition={{ duration: reduceMotion ? 0 : 0.22 }} className="overflow-hidden">
                <div className="timeline-details">
                    ...
                    <ul className="flex flex-wrap gap-1.5 mt-2" aria-label="Tecnologías">{exp.tech.map(tech => <li key={tech} className="text-[10px] px-2 py-1 rounded bg-secondary/10 text-on-surface-variant">{tech}</li>)}</ul>
                </div>
            </motion.div>
        </article>
    );
...
export default function ExperiencePage() {
    ...
    return (
        <section className="experience-page">
            <div className="neon-strip top-[12%] h-[76%] opacity-50" aria-hidden="true" />
            <motion.header initial={reduceMotion ? false : { opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="experience-header">
                <p className="font-mono text-secondary text-[10px] tracking-[0.3em] mb-1.5">// CAREER PATH</p>
                <h1 className="experience-title burning-text">Experience</h1>
                <p className="font-mono text-[10px] text-on-surface-variant/70 tracking-widest">MY PROFESSIONAL JOURNEY</p>
            </motion.header>
            <div className="timeline-stage" role="region" aria-label="Cronología profesional, del trabajo más reciente al más antiguo">
